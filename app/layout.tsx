...
import { JetBrains_Mono, Prompt, Source_Serif_4, Space_Grotesk } from "next/font/google";
...
const prompt = Prompt({ weight: ["400", "500", "600", "700"], subsets: ["latin"], variable: "--font-prompt", display: "swap" });
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="en"><body className={`${spaceGrotesk.variable} ${jetbrainsMono.variable} ${sourceSerif.variable} ${prompt.variable} flex min-h-screen flex-col`}><a href="#main" className="skip-link">Skip to content</a><Nav /><main id="main" className="flex-1">{children}</main><SiteFooter />{DialPanel && <DialPanel />}</body></html>;
}
