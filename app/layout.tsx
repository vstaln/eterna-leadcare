...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  return <html lang="en"><body className={`${spaceGrotesk.variable} ${jetbrainsMono.variable} v3 flex min-h-screen flex-col`}><a href="#main" className="skip-link">Skip to content</a><Nav /><main id="main" className="flex-1 overflow-x-hidden">{children}</main><SiteFooter />{DialPanel && <DialPanel />}</body></html>;
}
